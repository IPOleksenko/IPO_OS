...
build/toolchain/lua/lua.elf build/toolchain/lua/luac.elf: build/lib/libc.a build/lib/libm.a build/lib/libgcc.a build/lib/crt0.o build/lib/crti.o build/lib/crtn.o lib/linker.ld
	...
	cd apps/lua/src && make clean && make a CC="gcc -m32 -nostdinc -I$(CURDIR)/apps/include" AR="ar rcs" RANLIB="ranlib" MYCFLAGS="-DLUA_USE_POSIX -DLUA_USE_C89 -ffreestanding -fno-builtin"
...
build/toolchain/nasm.elf: build/lib/libc.a build/lib/libm.a build/lib/libgcc.a build/lib/crt0.o build/lib/crti.o build/lib/crtn.o lib/linker.ld
	...
	@mkdir -p apps/nasm/doc && touch apps/nasm/doc/Makefile.in
	$(MAKE) -C apps/nasm asm/nasm.o
	cd apps/nasm && ld -m elf_i386 -T $(CURDIR)/lib/linker.ld $(CURDIR)/build/lib/crt0.o $(CURDIR)/build/lib/crti.o asm/nasm.o libnasm.a $(CURDIR)/build/lib/libc.a $(CURDIR)/build/lib/libm.a $(CURDIR)/build/lib/libgcc.a $(CURDIR)/build/lib/crtn.o -o $(CURDIR)/$@

build/toolchain/python.elf: apps/micropython/micropython_shim.c build/lib/libc.a build/lib/libm.a build/lib/libgcc.a build/lib/crt0.o build/lib/crti.o build/lib/crtn.o lib/linker.ld
	@mkdir -p build/toolchain
	$(MAKE) -C apps/micropython/ports/unix VARIANT=minimal CFLAGS_EXTRA="-m32" LDFLAGS_EXTRA="-m32"
	gcc -m32 -c apps/micropython/micropython_shim.c -o build/toolchain/micropython_shim.o -Iapps/micropython -Iapps/micropython/ports/unix -Iapps/micropython/ports/unix/variants/minimal -Iapps/micropython/ports/unix/build-minimal -Iapps/micropython/ports/unix/build-minimal/genhdr
	ld -m elf_i386 -T lib/linker.ld build/lib/crt0.o build/lib/crti.o $$(find apps/micropython/ports/unix/build-minimal -name '*.o' ! -name 'printf.o' ! -name 'abort_.o' ! -name 'vfs_blockdev.o') build/toolchain/micropython_shim.o build/lib/libc.a build/lib/libm.a build/lib/libgcc.a build/lib/crtn.o -o $@
